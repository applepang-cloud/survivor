...
보완 후보(우선순위순):
1. **커트라인/소강의 리듬**: 지금은 페이즈가 단조 증가. 3분(밀도 스파이크)·5~7분(소강,
   튼튼몹 위주)·8분(최대 물량) 식의 계단+숨고르기 배치.
2. **스웜 이벤트**: 체력 1×80마리 직선 돌진(30초에 1회쯤, 2분부터) — 긴장+경험치 사탕.
3. **포위 이벤트**: 6분경 고체력 링 1회 — 동선 검사.
4. **진화 게이트 명시**: 진화 상자를 "4분 이후 보스"로 못박아 파워스파이크 시점 고정.
5. **사신 연출 강화**: 등장 10초 전 경고 + 1분마다 추가 스폰(현재 1마리).
6. **저주형 다이얼**(선택): 레벨업 패시브로 "저주(+스폰/+체력, 경험치×1.3)" 옵션 —
   숙련자용 리스크 교환.
