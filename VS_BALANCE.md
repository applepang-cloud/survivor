...
적용 현황 (2026-07-25 반영 → 2026-07-27 30분 표준화):
1. ✅ **VS 표준 30분 각본** — 14페이즈(mob_manager.dart):
   1:00 첫 분보스 → 5:00~5:40 약졸 스파이크 → 9:00 튼튼몹 커트라인 →
   10:00 진화 개방 → 13:00/21:00 포위 → 16:00~20:00 소강 → 24:00 램프 →
   **25:00 스테이지 보스** → 28:00~30:00 최대 물량 → **30:00 사신**.
2. ✅ **스웜 이벤트** — 2:00부터 45초마다 체력1×60마리(20×3열) 직선 관통,
   expDrop 0.3 (경험치 사탕), 접촉피해 3, 수명 후 조용히 소멸.
3. ✅ **포위 이벤트** — 13:00·21:00, 고체력(×5)·저속(16) 링 26마리 반경 560.
4. ✅ **진화 게이트** — 보물상자는 10:00 이후 보스만 드랍 (VS 규칙 그대로).
5. ✅ **사신 강화** — 29:50 경고 배너, 30:00부터 1분마다 +1 (무한 모드 제외).
6. ✅ **저주형 다이얼** — 장신구 '미치광이의 두개골'(💀, 최대 3랭크):
   랭크당 적 스폰 +25%·적 체력 +20%·경험치 +15% (리스크↔성장 교환).
7. ✅ **클리어 + 계속 도전** — 30:00 도달 시 STAGE CLEAR 팝업(+보너스 100🪙):
   "여기서 승리"(승리 결과 화면) 또는 "계속 도전"(사신 1분마다 추가되는 기록전).
   이후 사망해도 승리 화면으로 표시 (VS: 사신에게 죽어도 클리어 인정).
8. ✅ **25분 스테이지 보스** — 거대(×1.8)·고체력(1500×배율)·접촉 55,
   처치 시 확정 보물상자 + 금화 소나기 + **하이퍼 모드 해금** (VS 규칙).
9. ✅ **모드 3종 (VS식 해금 토글, 중첩 가능)** — 메뉴에서 선택:
   - 🔥 하이퍼: 적 체력·스폰 +50%, 골드 +50% — 해금: 25분 보스 처치
   - ⏩ 단축: 전체 2배속(HasTimeScale) + 경험치 +25% — 해금: 한 판 레벨 30
   - ♾️ 무한: 사신 없음, 30분 각본 루프, 사이클마다 체력 +100%·스폰 +50%
     — 해금: 30분 클리어. 해금은 shared_preferences로 영구 저장.
